...
func norm_manhattan(_ p: Point) -> Int {
    return abs(p.x) + abs(p.y) + abs(p.z)
}

var max_dist = 0
for aligned_1 in aligned_pointclouds {
    let (origin_1, _) = aligned_1!
    for aligned_2 in aligned_pointclouds {
        let (origin_2, _) = aligned_2!
        let dist = norm_manhattan(sub(origin_1, origin_2))
        max_dist = max(max_dist, dist)
    }
}

print("Largest distance between two scanners is \(max_dist)")
