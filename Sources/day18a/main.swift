...
func add(_ lhs: SnailfishNumber, _ rhs: SnailfishNumber) -> SnailfishNumber {
    let sum = SnailfishNumber(.pair(lhs, rhs), parent: nil)
    lhs.parent = sum
    rhs.parent = sum
...
while let line = readLine() {
    ...
    if let previous_sum = sum {
        sum = add(previous_sum, number)
    } else {
        sum = number
    }
}

print("The magnitude of the final sum is \(magnitude(sum!))")
