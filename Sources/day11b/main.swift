...
var first_step_when_synchronized: Int? = nil
for step in 1... {
    let flashes = simulate_one_step()
    if flashes == map_height * map_width {
        first_step_when_synchronized = step
        break
    }
}

print("\(first_step_when_synchronized!) is the first step during which all octopuses flash")
